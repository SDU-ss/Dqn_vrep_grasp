...
class Dqn():
    ...
    def choose_action_for_eval(self, state_path):
        ...
        state_depth = np.array(state_depth)
        # normlize
        state_depth = (state_depth - Train_Configs.MIN_HEIGHTMAP_ARR) / (Train_Configs.MAX_HEIGHTMAP_ARR - Train_Configs.MIN_HEIGHTMAP_ARR)#(state_depth - Train_Configs.MIN_DEPTH_ARR) / (Train_Configs.MAX_DEPTH_ARR - Train_Configs.MIN_DEPTH_ARR)
        # numpy to tensor
        state_rgb = torch.cuda.FloatTensor(state_rgb)  # dim:[INPUT_IMAGE,3,224,224]
        state_depth = torch.cuda.FloatTensor(state_depth)  # dim:[INPUT_IMAGE,3,224,224]
...
if __name__ == '__main__':
    ...
    start_step = 10000
    end_step =220000
    # evaluation('step_30000_params.pkl')
